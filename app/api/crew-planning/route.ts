...
async function ensureCrewPlanning() {
  await ensureDatabase(); const d1 = env.DB;
  await d1.batch([
...
export async function GET(request: Request) {
  ...
  const [plans, assignments, users] = await Promise.all([
    env.DB.prepare("SELECT flight_no AS flightNo, base_cabin_crew AS baseCabinCrew, lead_crew AS leadCrew, additional_crew AS additionalCrew, double_crew AS doubleCrew, required_total AS requiredTotal, updated_by AS updatedBy, updated_at AS updatedAt FROM crew_plans").all(),
    env.DB.prepare("SELECT id, flight_no AS flightNo, user_id AS userId, crew_name AS crewName, employee_id AS employeeId, assignment_role AS assignmentRole, status, created_at AS createdAt FROM crew_assignments ORDER BY assignment_role DESC, crew_name").all(),
    env.DB.prepare("SELECT id, full_name AS fullName, employee_id AS employeeId, role, station FROM crew_users WHERE status = 'active' ORDER BY full_name").all(),
  ]);
...
export async function POST(request: Request) {
  ...
  if (body.action === "updatePlan") {
    const flightNo = String(body.flightNo ?? ""); const flight = await env.DB.prepare("SELECT aircraft FROM flights WHERE flight_no = ?").bind(flightNo).first<{ aircraft: string }>();
    ...
    await env.DB.prepare("UPDATE crew_plans SET base_cabin_crew = ?, lead_crew = ?, additional_crew = ?, double_crew = ?, required_total = ?, updated_by = ?, updated_at = ? WHERE flight_no = ?").bind(rule.baseCabinCrew, rule.leadCrew, additional, doubleCrew ? 1 : 0, requiredTotal, user.fullName, new Date().toISOString(), flightNo).run();
  ...
  if (body.action === "assignCrew") {
    ...
    const plan = await env.DB.prepare("SELECT required_total AS requiredTotal FROM crew_plans WHERE flight_no = ?").bind(flightNo).first<{ requiredTotal: number }>(); const count = await env.DB.prepare("SELECT COUNT(*) AS total FROM crew_assignments WHERE flight_no = ?").bind(flightNo).first<{ total: number }>();
    ...
    try { const result = await env.DB.prepare("INSERT INTO crew_assignments (flight_no, user_id, crew_name, employee_id, assignment_role, status) VALUES (?, ?, ?, ?, ?, 'confirmed')").bind(flightNo, body.userId ? Number(body.userId) : null, crewName, employeeId, role).run(); return Response.json({ id: result.meta.last_row_id }, { status: 201 }); } catch { return Response.json({ error: "This crew member is already assigned to the flight." }, { status: 409 }); }
  }
  return Response.json({ error: "Unsupported action" }, { status: 400 });
}

export async function DELETE(request: Request) {
  const user = await getCrewUser(request); if (!user) return unauthorized(); await ensureCrewPlanning(); const id = Number(new URL(request.url).searchParams.get("id")); if (!id) return Response.json({ error: "Assignment id required" }, { status: 400 }); await env.DB.prepare("DELETE FROM crew_assignments WHERE id = ?").bind(id).run(); return Response.json({ ok: true });
}
