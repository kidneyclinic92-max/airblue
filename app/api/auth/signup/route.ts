...
export async function POST(request: Request) {
  ...
  const allowedRoles = ["Flight Attendant", "Cabin Supervisor", "Senior Purser", "Catering Staff", "Catering Supervisor"];
  const role = allowedRoles.includes(body.role ?? "") ? String(body.role) : "Flight Attendant";
  ...
  const result = await database.prepare("INSERT INTO crew_users (full_name, employee_id, email, role, station, password_hash, password_salt, password_iterations) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").bind(fullName, employeeId, email, role, body.station?.toUpperCase() ?? "ISB", hashed.hash, hashed.salt, hashed.iterations).run();
  const userId = Number(result.meta.last_row_id); const cookie = await createSession(userId, request);
  return Response.json({ user: { id: userId, fullName, employeeId, email, role, station: body.station ?? "ISB" } }, { status: 201, headers: { "set-cookie": cookie } });
}
