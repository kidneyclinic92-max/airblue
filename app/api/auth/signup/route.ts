...
export async function POST(request: Request) {
  ...
  const exists = await env.DB.prepare("SELECT id FROM crew_users WHERE email = ? OR employee_id = ?").bind(email, employeeId).first();
  if (exists) return Response.json({ error: "An account already exists for this email or employee ID." }, { status: 409 });
  const hashed = await hashPassword(password);
  const result = await env.DB.prepare("INSERT INTO crew_users (full_name, employee_id, email, role, station, password_hash, password_salt, password_iterations) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").bind(fullName, employeeId, email, body.role ?? "Flight Attendant", body.station?.toUpperCase() ?? "ISB", hashed.hash, hashed.salt, hashed.iterations).run();
